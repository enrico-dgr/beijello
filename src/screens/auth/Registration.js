...
import { checkEmail, checkPassword, isEmpty, capitalizeFirstLetter } from '../../utils/utils';
    ...
    const checkReg = () => {
        ...
        for (const key in user) {
            if (Object.hasOwnProperty.call(user, key)) {
                ...
                if (isEmpty(user[key])) {
                    error[key] = true;
                    errorI18n[key] = `Empty${capitalizeFirstLetter(key)}`
        ...
        if(!isThereAnyError){
            //  name, surname, gender, phone, email, password, maritalStatus
            const signUpError = signUp({
            ...
            if(signUpError.emailUsed){
                errorI18n.email = "UsedEmail" 
                error.email = true
            }

            if (signUpError.errorMessage!=='') {
                navigate('/')
            }
        }
    ...
    return (
        <div className="auth-default-form auth-signup-form">
            ...
            <div className='goto-sign goto-sign--in'  >
                <span style={{ cursor: 'pointer' }} onClick={onSignUpClick}>{t('Registration.SignIn')}</span>
            </div>
            <Input
                height={60}
                className="name"
            ...
            <Input
                height={60}
                className={`birth-date ${state.user.date===''?'':'birth-date--filled'}`}
                label="Birth date"
                type="date"
            ...
            <Input
                ...
                errorText={t(`Registration.EmailErrors.${state.errorI18n.email}`)}
            />
            <Input
                height={60}
                label="Password"
                ...
                errorText={t(`Registration.PasswordErrors.${state.errorI18n.password}`)}
            />
            <div className='form-marital-status'>
                <label>{t("Registration.MaritalStatus.Name")}
                    <select value={'single'} onChange={handleChange}>
                        <option value="single">{t("Registration.MaritalStatus.Single")}</option>
                        <option value="married">{t("Registration.MaritalStatus.Married")}</option>
                        <option value="divorced">{t("Registration.MaritalStatus.Divorced")}</option>
                        <option value="self-married">{t("Registration.MaritalStatus.SelfMarried")}</option>
                    </select>
                </label>
            </div>
            <SubmitButton
