...
const WorkSpaceModal = (props) => {
    ...
    const addNewWorkspace = () => {
        ...
        addWorkSpace(ob);
        props.dispatch(
            setWorkspaces(getWorkSpacesByEmail(props.email))
        );
        if (props.callBackHideModal !== undefined) {
            props.callBackHideModal()
        }
    ...
    return (
        <>
            <Modal>
                ...
                <SubmitButton onClick={handlerHideModal} label="annulla"></SubmitButton>
                <SubmitButton onClick={addNewWorkspace} label="crea "></SubmitButton>
            </Modal>
        </>
    )
}
